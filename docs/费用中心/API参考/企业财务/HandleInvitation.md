...
[去调试](https://api.volcengine.com/api-explorer/?action=HandleInvitation&groupName=%E4%BC%81%E4%B8%9A%E8%B4%A2%E5%8A%A1&serviceCode=billing&version=2022-01-01)
...
| 参数名称 | 类型 | 是否必选 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| Action | string | 必选 | HandleInvitation | 要执行的操作，取值：HandleInvitation。 |
| Version | string | 必选 | 2022-01-01 | API的版本，取值：2022-01-01。 |
| InvitationID | string | 必选 | inv-xxxxx | 邀约ID |
| Action | integer | 必选 | 1 | 操作类型：1-接受，2-拒绝 |
...
```text
curl --location 'https://open.volcengineapi.com?Action=HandleInvitation&Version=2022-01-01' \
...
| 400 | InvitationExpired | Invitation has expired | 邀约已过期 |
