# ListAmortizedCostBillDaily - 分页查询成本账单按天
...
[去调试](https://api.volcengine.com/api-explorer/?action=ListAmortizedCostBillDaily&groupName=%E8%B4%A6%E5%8D%95%E4%B8%AD%E5%BF%83&serviceCode=billing&version=2022-01-01)
...
| 参数名称 | 类型 | 是否必选 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| Action | string | 必选 | ListAmortizedCostBillDaily | 要执行的操作，取值：ListAmortizedCostBillDaily。 |
| Version | string | 必选 | 2022-01-01 | API的版本，取值：2022-01-01。 |
| AmortizedDay | string | 必选 | 2023-08-01 | 成本日期：格式为YYYY-MM-DD；仅支持单日查询 |
...
| 参数名称 | 类型 | 示例值 | 描述 |
| --- | --- | --- | --- |
| List | object[] | - | 成本账单按天列表 |
| List.AmortizedDay | string | 2023-08-01 | 成本日期 |
...
| List.OriginalAmortizedAmount | string | 5.58 | 分摊原价 |
| List.DiscountAmortizedAmount | string | 5.00 | 分摊折后价 |
| List.CouponAmortizedAmount | string | 0.00 | 分摊代金券 |
| List.PayableAmortizedAmount | string | 5.00 | 分摊应付金额 |
...
```text
curl --location 'https://open.volcengineapi.com?Action=ListAmortizedCostBillDaily&Version=2022-01-01' \
...
```json
{
    "ResponseMetadata": {
        ...
        "Action": "ListAmortizedCostBillDaily",
        "Version": "2022-01-01",
        "Service": "billing",
        "Region": "cn-north-1"
    },
        ...
        "List": [
            {
                ...
                "AmortizedType": "按天分摊",
                "BillPeriod": "2022-10",
                "OriginalAmortizedAmount": "5.58",
                "DiscountAmortizedAmount": "5.00",
                "CouponAmortizedAmount": "0.00",
                "PayableAmortizedAmount": "5.00"
            }
        ],
        "Total": 1,
        "Limit": 10,
