...
[去调试](https://api.volcengine.com/api-explorer/?action=UpdateAuth&groupName=%E4%BC%81%E4%B8%9A%E8%B4%A2%E5%8A%A1&serviceCode=billing&version=2022-01-01)
...
| 参数名称 | 类型 | 是否必选 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| Action | string | 必选 | UpdateAuth | 要执行的操作，取值：UpdateAuth。 |
...
| AuthInfoList | object[] | 必选 | - | 授权信息列表 |
| AuthInfoList.AuthID | string | 必选 | - | 授权ID |
| AuthInfoList.AuthStatus | integer | 必选 | - | 授权状态：1-开启，0-关闭 |
...
```text
curl --location 'https://open.volcengineapi.com?Action=UpdateAuth&Version=2022-01-01' \
...
| 400 | AuthNotFound | Auth not found | 授权不存在 |
