...
[去调试](https://api.volcengine.com/api-explorer/?action=CreateFinancialRelation&groupName=%E4%BC%81%E4%B8%9A%E8%B4%A2%E5%8A%A1&serviceCode=billing&version=2022-01-01)
...
| 参数名称 | 类型 | 是否必选 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| Action | string | 必选 | CreateFinancialRelation | 要执行的操作，取值：CreateFinancialRelation。 |
...
| AuthInfoList | object[] | 可选 | - | 授权信息列表 |
| AuthInfoList.AuthID | string | 可选 | - | 授权ID |
| AuthInfoList.AuthStatus | integer | 可选 | - | 授权状态 |
...
| InvitationID | string | inv-xxxxx | 邀约ID |

## 请求示例

```text
curl --location 'https://open.volcengineapi.com?Action=CreateFinancialRelation&Version=2022-01-01' \
